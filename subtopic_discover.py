...
OUTPUT = os.path.join(DATA_DIR, "subtopics_discovered.json")
...
def load_data():
    ...
    guardian = load_paired('data/guardian_articles_raw.json', 'data/news_guardian_classified.json',
        'url', lambda r: f"Title: {r.get('title','')}\nSummary: {r.get('trail_text','')}")
    nyt = load_paired('data/nyt_articles_raw.json', 'data/news_nyt_classified.json',
        'url', lambda r: f"Title: {r.get('title','')}\nSummary: {r.get('abstract','')}")
    all_en = guardian + nyt

    with open('data/naver_articles_raw.json', encoding='utf-8') as f:
        naver_raw = json.load(f)
    with open('data/news_naver_classified.json', encoding='utf-8') as f:
        naver_cls = json.load(f)
    naver_by_id = {a['link']: a for a in naver_raw}
    naver_paired = []
    for c in naver_cls:
        r = naver_by_id.get(c['article_id'])
        if r:
            naver_paired.append({
                'text': f"제목: {r.get('title','')}\n요약: {r.get('description','')}",
                'primary': c.get('primary', 'none')
            })
    ...
    for a in naver_paired:
        canon = KR_TO_EN.get(a['primary'].strip())
        if canon:
            kr_groups[canon].append(a['text'])
