...
def main():
    ...
    db_holder = [duckdb.connect(config.DB_PATH)]
    con = db_holder[0]
    init_db(con)
    ...
    db_mb_start = os.path.getsize(config.DB_PATH) / 1024 / 1024 if os.path.exists(config.DB_PATH) else 0
    ...
    db_mb_end = os.path.getsize(config.DB_PATH) / 1024 / 1024 if os.path.exists(config.DB_PATH) else 0
    print_job_footer(
        done=done,
        skipped_cached=skipped_cached,
        skipped_empty=skipped_empty,
