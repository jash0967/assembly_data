...
data/docs/{source}/{doc_id}.{ext}. Text is extracted and stored in
...
HWP5TXT = str(ROOT / "venv" / "Scripts" / "hwp5txt.exe")
...
def _reader_con():
    ...
    return _db_con if _db_con is not None else duckdb.connect(config.DB_PATH, read_only=True)
