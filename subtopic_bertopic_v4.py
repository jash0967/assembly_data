"""BERTopic v4: 언어 분리 + 의미 정렬(cross-lingual alignment).

v3 대비 추가:
- EN/KO 토픽 센트로이드 코사인 유사도로 대응 토픽 병합
- 결과: 병합 토픽 / EN 고유 토픽 / KO 고유 토픽 구분

Usage:
    python subtopic_bertopic_v4.py --attr 책임/윤리AI
    python subtopic_bertopic_v4.py
"""
...
OUTPUT   = os.path.join(DATA_DIR, "subtopics_bertopic_v4.json")
client   = OpenAI()
...
def load_news():
    ...
    for a in nv_raw:
        ...
        for key in ["link","originallink"]:
            aid = a.get(key,"")
            if aid: nv_map[aid] = text

    articles = []
    for cls_file, source, lang, text_map, normalize in [
        ("news_guardian_classified.json","guardian","en",g_map,True),
        ("news_nyt_classified.json","nyt","en",n_map,True),
        ("news_naver_classified.json","naver","ko",nv_map,False),
    ]:
