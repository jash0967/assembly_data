"""소스별 정책 속성별 기사 제목 리스트 (Guardian / NYT).
...
SOURCES = {
    "guardian": {
        "raw_dir":  config.NEWS_DIR,
        "raw":      "guardian_articles_raw.json",
        "cls_dir":  config.ANALYSIS_DIR,
        "cls":      "articles_classified_guardian.json",
    ...
    "nyt": {
        ...
        "cls_dir":  config.ANALYSIS_DIR,
        "cls":      "articles_classified_nyt.json",
...
def export(source):
    cfg = SOURCES[source]
    raw_path = os.path.join(cfg["raw_dir"], cfg["raw"])
    cls_path = os.path.join(cfg["cls_dir"], cfg["cls"])
    out_path = os.path.join(config.EXPORTS_DIR, cfg["out"])
