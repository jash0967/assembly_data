...
def main():
    with open(os.path.join(config.ANALYSIS_DIR, 'subtopics_discovered.json'), encoding='utf-8') as f:
        data = json.load(f)
    with open(os.path.join(config.ANALYSIS_DIR, 'subtopics_final.json'), encoding='utf-8') as f:
        final = json.load(f)
...
Respond with JSON: {{"matches": [{{"subtopic": "...", "count": N, "matched_seeds": [1,2,3]}}]}}"""
    ...
    output = os.path.join(config.ANALYSIS_DIR, 'subtopics_overlap_rate.json')
